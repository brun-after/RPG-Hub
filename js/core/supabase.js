...
// ── SUPABASE FETCH ────────────────────────────────────────────
async function sb(path, opts={}, _retry=0) {
...
async function _carregarProgressivo(rpgId) {
 ...
 setTimeout(async ()=>{
   try {
     ...
     if(curEntry && curEntry.mapa.img_url) renderMapaViewer(curEntry.mapa.map_id);
...
async function importRPG(payload, mapasJSON=null){
 ...
 const registryBody={rpg_id:rpgId,name:cfg.name,owner_id:ownerId,theme_json:theme};
