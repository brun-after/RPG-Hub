...
// ═══════════════════════════════════════════════════════════════════════════
var APMOD_PARTS: Record<string, any> = {
cabelo:[],
rosto:[],
camisa:[],
calca:[],
...
var CHAR_JSON_TEMPLATES: any[]=[];
const EQUIP_SLOT_LIMITS: Record<string, any> = {arma_1m:{maxW:30,maxH:60,label:'Arma 1 Mão'},arma_2m:{maxW:40,maxH:70,label:'Arma 2 Mãos'},escudo:{maxW:28,maxH:32,label:'Escudo'},elmo:{maxW:20,maxH:20,label:'Elmo'},capa:{maxW:28,maxH:44,label:'Capa'},amuleto:{maxW:12,maxH:14,label:'Amuleto'},anel:{maxW:8,maxH:8,label:'Anel'},arco:{maxW:10,maxH:60,label:'Arco'},lanca:{maxW:10,maxH:70,label:'Lança'},geral:{maxW:24,maxH:40,label:'Geral'}};
...
function _hexDarken2(hex: any,a: any){try{let c=hex.replace('#','');if(c.length===3)c=c[0]+c[0]+c[1]+c[1]+c[2]+c[2];const n=parseInt(c,16);return '#'+((1<<24)+(Math.max(0,(n>>16)-a)<<16)+(Math.max(0,((n>>8)&0xff)-a)<<8)+Math.max(0,(n&0xff)-a)).toString(16).slice(1);}catch(e){return hex;}}

// ── Helpers de renderização SVG ───────────────────────────────────────────
function _hexDarken(hex: any, amount: any) {
  try {
    let c = hex.replace('#','');
...
function _svgPart(template: any, c: any, c2: any) {
  if (!template) return '';
...
function apmodCarregarTemplate(id: any){
  ...
  tipos.forEach(tipo=>{const pid=t.partes[tipo];if(pid&&!(APMOD_PARTS&&(APMOD_PARTS[tipo]||[]).find((p: any)=>p.id===pid))){console.warn(`Template ${t.id}: parte ${tipo}="${pid}" não existe`);temParteFaltando=true;}});
...
function apmodAtualizarPreview(){
  ...
  if(ap.modo==='imagem'){
    ...
    const _pvEqLayer=(camada: any)=>equips.filter((eq: any)=>eq.visivel!==false&&(eq.img||eq.img_url||(eq.svg&&eq.svg.length>5))&&(camada==='atras'?eq.camada==='atras':eq.camada!=='atras')).map((eq: any)=>{
  ...
  if(prevIso){
    ...
    const _pvEqOv=(camada: any)=>_pvEquips.filter((eq: any)=>eq.visivel!==false&&(eq.img||eq.img_url||(eq.svg&&eq.svg.length>5))&&(camada==='atras'?eq.camada==='atras':eq.camada!=='atras')).map((eq: any)=>{
      ...
      const rot=eq.rotacao||0,rotH=eq.rotacaoH||0;
      const _pvWarp=eq.warpCorners?_aeqComputeMatrix3d(eW,eH,eq.warpCorners.map((c: any)=>({x:c.x*eW,y:c.y*eH}))):null;
  ...
  if(prevMini){
    ...
    const _mnEqOv=(camada: any)=>_mnEquips.filter((eq: any)=>eq.visivel!==false&&(eq.img||eq.img_url||(eq.svg&&eq.svg.length>5))&&(camada==='atras'?eq.camada==='atras':eq.camada!=='atras')).map((eq: any)=>{
      ...
      const rot=eq.rotacao||0,rotH=eq.rotacaoH||0;
      const _mnWarp=eq.warpCorners?_aeqComputeMatrix3d(eW,eH,eq.warpCorners.map((c: any)=>({x:c.x*eW,y:c.y*eH}))):null;
...
function apmodTogglePreviewGrande(triggerEl: any) {
...
function apmodSharpenImg(imgEl: any){
...
async function apmodFileToBase64(input: any, targetId: any) {
...
async function _aeqGenerateComposedImg(aparencia: any, equipVisuais: any, charNome: any) {
  try {
    const W = 240, H = 360;
    ...
    // ──────────────────────────────────────────────────────────────────────
    function isIdentityWarp(corners: any) {
    ...
    // ──────────────────────────────────────────────────────────────────────
    function loadImg(src: any, isSvg: any, w: any, h: any) {
    ...
    // ──────────────────────────────────────────────────────────────────────
    function drawImageWarped(img: any, srcW: any, srcH: any, corners: any) {
      const N = 20;
      const lerp = (a: any, b: any, t: any) => a + (b - a) * t;
      const dp = (u: any, v: any) => ({
      ...
      function tri(x0: any,y0: any,x1: any,y1: any,x2: any,y2: any, u0: any,v0: any,u1: any,v1: any,u2: any,v2: any) {
    ...
    // ──────────────────────────────────────────────────────────────────────
    async function drawEquipLayer(camada: any) {
      const equips = (equipVisuais || []).filter((eq: any) =>
        eq.visivel !== false &&
...
async function apmodSalvar(nome: any){
  const ap=apmodGetCurrentAparencia();
  ...
  const _somarEquipBonus=(equips: any)=>{const soma: Record<string, any> = {};(equips||[]).forEach((eq: any)=>{if(!eq.bonus_attrs)return;Object.entries<any>(eq.bonus_attrs).forEach(([k,v])=>{soma[k]=(soma[k]||0)+v;});});return soma;};
...
// ── UI da aba Tint ────────────────────────────────────────────────────────
function _apmodTabTint(aparencia: any) {
  ...
  const linhasHtml = tints.length
    ? tints.map((t: any, i: any) => _apmodTintLinhaHtml(i, t, modosHtml)).join('')
...
function _apmodTintLinhaHtml(i: any, t: any, modosHtml: any) {
...
function apmodTintIniciar(aparencia: any) {
  window._apmodTints = JSON.parse(JSON.stringify(aparencia.tints || []));
  apmodTintRefresh();
}
...
function apmodTintAtualizar(i: any, campo: any, valor: any) {
...
function apmodTintRemover(i: any) {
...
function apmodTintRefresh() {
  ...
  lista.innerHTML = window._apmodTints.length
    ? window._apmodTints.map((t: any, i: any) => _apmodTintLinhaHtml(i, t, modosHtml)).join('')
...
function apmodRemoverEquip(idx: any){if(idx<0)return;window._apmodEquipsVisuais.splice(idx,1);_apmodRefreshEquipLista();}

function _apmodTabEquip(aparencia: any,nome: any){
  const equips=aparencia.equipamentos_visuais||[];
  const _ri=(eq: any,i: any)=>{
    const bonusStr=eq.bonus_attrs&&Object.keys(eq.bonus_attrs).length?'📊 '+Object.entries<any>(eq.bonus_attrs).map(([k,v])=>k+(v>0?'+':'')+v).join(' · '):'';
    const unlocksStr=eq.unlock_efeitos&&eq.unlock_efeitos.efeitos&&eq.unlock_efeitos.efeitos.length?'🔓 '+eq.unlock_efeitos.efeitos.map((e: any)=>e.nome||e.tipo).join(', '):'';
  ...
  return '<div id="apmod-tab-equip" class="apmod-tab-content" style="display:none">'
    ...
    +'<div id="apmod-equip-lista" style="margin-bottom:12px">'+(equips.length?equips.map((eq: any,i: any)=>_ri(eq,i)).join(''):'<div style="color:var(--suave);font-style:italic;font-size:0.82rem;text-align:center;padding:20px 0">Nenhum equipamento visual</div>')+'</div>'
...
function _apmodRefreshEquipLista(){
  ...
  lista.innerHTML=equips.map((eq: any,i: any)=>{
    const bonusStr=eq.bonus_attrs&&Object.keys(eq.bonus_attrs).length?'📊 '+Object.entries<any>(eq.bonus_attrs).map(([k,v])=>k+(v>0?'+':'')+v).join(' · '):'';
    const unlocksStr=eq.unlock_efeitos&&eq.unlock_efeitos.efeitos&&eq.unlock_efeitos.efeitos.length?'🔓 '+eq.unlock_efeitos.efeitos.map((e: any)=>e.nome||e.tipo).join(', '):'';
    const temImg=!!(eq.img||eq.img_url);const temSvg=!!(eq.svg&&eq.svg.length>5);
    const isAtras=eq.camada==='atras';
...
function apmodToggleEquipCamada(idx: any){
...
function apmodAbrirAdicionarEquip(editIdx: any) {
...
function _aeqComputeMatrix3d(srcW: any, srcH: any, dst: any) {
  // Validar corners — se algum for inválido ou muito extremo, não aplica warp
  if (!dst || dst.length < 4 || dst.some((p: any) => !isFinite(p.x) || !isFinite(p.y) || Math.abs(p.x) > srcW * 2.5 || Math.abs(p.y) > srcH * 2.5)) return 'none';
  function adj(m: any){return[m[4]*m[8]-m[5]*m[7],m[2]*m[7]-m[1]*m[8],m[1]*m[5]-m[2]*m[4],m[5]*m[6]-m[3]*m[8],m[0]*m[8]-m[2]*m[6],m[2]*m[3]-m[0]*m[5],m[3]*m[7]-m[4]*m[6],m[1]*m[6]-m[0]*m[7],m[0]*m[4]-m[1]*m[3]];}
  function mul(a: any,b: any){const c=Array(9).fill(0);for(let i=0;i<3;i++)for(let j=0;j<3;j++)for(let k=0;k<3;k++)c[3*i+j]+=a[3*i+k]*b[3*k+j];return c;}
  function mv(m: any,v: any){return[m[0]*v[0]+m[1]*v[1]+m[2]*v[2],m[3]*v[0]+m[4]*v[1]+m[5]*v[2],m[6]*v[0]+m[7]*v[1]+m[8]*v[2]];}
  function basis(pts: any){const m=[pts[0].x,pts[1].x,pts[2].x,pts[0].y,pts[1].y,pts[2].y,1,1,1];const v=mv(adj(m),[pts[3].x,pts[3].y,1]);return[m[0]*v[0],m[1]*v[1],m[2]*v[2],m[3]*v[0],m[4]*v[1],m[5]*v[2],m[6]*v[0],m[7]*v[1],m[8]*v[2]];}
...
function _aeqRepaintWarpLayer(corners: any, iW: any, iH: any) {
  const hSize = 18;
...
function _aeqWarpGridInner(corners: any, iW: any, iH: any) {
  const n = 7;
  const lp = (a: any, b: any, t: any) => a + (b - a) * t;
  const px = (u: any, v: any) => {
...
function _aeqBuildWarpLayer(corners: any, iW: any, iH: any) {
  const drag = document.getElementById('aeq-drag'); if (!drag) return;
...
function _aeqWarpMoveDoc(e: any) {
...
async function aeqFileUpload(inp: any) {
...
(function() {
  ...
  apmodRenderFront = function(this: any, aparencia, corBase) {
    ...
    const hdColors: Record<string, any> = {cabelo: [corCab, _hexDarken(corCab,30)], rosto: [corOlho, _hexDarken(corOlho,40)],
    ...
    for (const tipo of hdParts) {
      const id = p[tipo];
      if (!id) continue;
      const part = (APMOD_PARTS[tipo]||[]).find((x: any) => x.id === id);
...
Object.defineProperty(globalThis, "APMOD_PARTS", { configurable: true, get: () => APMOD_PARTS, set: (__v) => { APMOD_PARTS = __v; } });
