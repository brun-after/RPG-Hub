...
function _htmlControleMobile() {
  ...
  return `
    ...
    ${emAvtDisp ? `<button id="mc-inv-btn" ontouchend="event.preventDefault();avtAbrirInventario()" onclick="avtAbrirInventario()" aria-label="Abrir inventário" style="position:fixed;top:8px;left:202px;z-index:9202;pointer-events:auto;padding:3px 8px;background:rgba(200,168,75,0.12);border:1px solid rgba(200,168,75,0.3);border-radius:6px;color:rgba(200,168,75,0.85);font-family:var(--fonte-d);font-size:0.5rem;cursor:pointer;touch-action:manipulation">🎒 Inv</button>` : ''}
