...
async function aprovarCriativoCompleto() {
  ...
  try {
    ...
    // Verificar se o mestre deve executar imediatamente
    // (NPC, personagem sem jogador vinculado, personagem vinculado ao mestre, ou jogador ausente)
    if (criativo) {
      const atacante    = criativo.atacante;
      const chars       = RPG_DATA?.characters || [];
      const char        = chars.find(c => c.nome === atacante);
      const isNpc       = char?.custom_attrs?.tipo_personagem === 'npc';
      ...
      if (mestreExecuta) {
        // Mestre executa na hora — não precisa aguardar o jogador
        setTimeout(() => abrirModalExecucaoCriativo(criativoId), 150);
