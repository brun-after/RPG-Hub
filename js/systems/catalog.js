...
function nmceFullscreenAbrir() {
  ...
  // Mover canvas (e guide) para dentro do wrap fullscreen
...
// ── CANVAS EDITOR: grade guia ISO ────────────────────────────
// Renderiza sobre o canvas de guia (não é exportado com o mapa)
function nmceUpdateIsoGuide() {
  const guide = document.getElementById('nmce-guide');
  const note  = document.getElementById('nmce-iso-note');
  if (!guide) return;
  const tipo = document.getElementById('nm-tipo')?.value;
  const isLocal = tipo === 'local';

  guide.style.display = isLocal ? 'block' : 'none';
  ...
  _drawIsoGrid(ctx, W, H, 40, 'rgba(126,200,240,0.55)');
}
...
// ════════════════════════════════════════════════════════════════════════════
const ATTR_MAPPING_CACHE = {}; // rpgId → [{ nome_customizado, grupo_base }]
