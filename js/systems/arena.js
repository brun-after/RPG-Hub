...
async function arCarregarTudo() {
  ...
  try {
    ...
    if (AR.estado.iniciativa_arena) AR.iniciativa = AR.estado.iniciativa_arena;
...
function renderArenaEfeitos() {
  ...
  const efeitos = Object.values(efeitosMap);
...
function renderArenaConfig() {
  ...
  el.innerHTML = AR.histList.map(h => {
    let hd = {}; try { hd = JSON.parse(h.conteudo||'{}'); } catch(e) {}
    return `<div class="ar-hist-item" onclick="verHistorico(${h.id})">
      <div class="ar-hist-nome">${h.titulo}</div>
      <div class="ar-hist-meta">${hd.chars_count||0} personagens · ${hd.turno_final||0} turnos · ${hd.data||''}</div>
    </div>`;
  }).join('');
}
...
async function salvarChar() {
  ...
  customAttrs.descricao = desc;
  customAttrs.tipo = tipo;
  customAttrs.cor = cor;
  customAttrs.img_url = normalizeImgUrl(img);
  customAttrs.hp_max = hpMax;
  if (!customAttrs.buffs) customAttrs.buffs = [];
  // Salvar owner para controle de edição
  if (!isEdit && tipo === 'jogador') {
    customAttrs.owner_nickname = AR.myNickname || (SESSION?.profile?.nickname || '');
  }
  // Habilidades de NPC
  if (tipo === 'criatura') {
    customAttrs.habilidades = NPC_HABILIDADES_TEMP;
  } else {
    delete customAttrs.habilidades;
  }
  NPC_HABILIDADES_TEMP = [];
  // Init position at random spot if new char
  if (!isEdit || !customAttrs.pos) {
...
async function salvarEfeito() {
  ...
  if (document.getElementById('ar-ef-heal-on').checked) {
    efBase.heal_formula = document.getElementById('ar-ef-heal-formula').value.trim() || '20';
  }
  if (document.getElementById('ar-ef-hot-on').checked) {
    efBase.hot_formula = document.getElementById('ar-ef-hot-formula').value.trim() || '1d6';
    efBase.hot_turnos  = parseInt(document.getElementById('ar-ef-hot-turnos').value)||3;
    efBase.hot_turnos_restantes = efBase.hot_turnos;
  }
  if (document.getElementById('ar-ef-boost-on').checked) {
    efBase.boost_dano        = parseInt(document.getElementById('ar-ef-boost-mod').value)||5;
    efBase.boost_dano_turnos = parseInt(document.getElementById('ar-ef-boost-turnos').value)||2;
    efBase.boost_dano_turnos_restantes = efBase.boost_dano_turnos;
  }
  if (document.getElementById('ar-ef-rec-on').checked) {
    efBase.rec_atributo    = document.getElementById('ar-ef-rec-atributo').value.trim();
    efBase.rec_formula     = document.getElementById('ar-ef-rec-formula').value.trim() || '10';
    efBase.rec_modo        = document.getElementById('ar-ef-rec-modo').value;
    efBase.rec_turnos      = parseInt(document.getElementById('ar-ef-rec-turnos').value)||3;
    efBase.rec_turnos_restantes = efBase.rec_modo === 'turno' ? efBase.rec_turnos : 0;
  }
  // Negativos
  if (document.getElementById('ar-ef-dot-on').checked) {
    efBase.dot_formula = document.getElementById('ar-ef-dot-formula').value.trim() || '1d6';
    efBase.dot_turnos  = parseInt(document.getElementById('ar-ef-dot-turnos').value)||3;
    efBase.dot_turnos_restantes = efBase.dot_turnos;
  }
  if (document.getElementById('ar-ef-deb-on').checked) {
    efBase.mod_dano        = parseInt(document.getElementById('ar-ef-deb-mod').value)||-4;
    efBase.mod_dano_turnos = parseInt(document.getElementById('ar-ef-deb-turnos').value)||2;
    efBase.mod_dano_turnos_restantes = efBase.mod_dano_turnos;
  }
  if (document.getElementById('ar-ef-mov-on').checked) {
    efBase.sem_movimento = true;
    efBase.sem_movimento_turnos = parseInt(document.getElementById('ar-ef-mov-turnos').value)||1;
    efBase.sem_movimento_turnos_restantes = efBase.sem_movimento_turnos;
  }
  if (document.getElementById('ar-ef-atk-on').checked) {
    efBase.sem_ataque      = true;
    efBase.sem_ataque_tipo = document.getElementById('ar-ef-atk-tipo').value || 'todos';
    efBase.sem_ataque_turnos = parseInt(document.getElementById('ar-ef-atk-turnos').value)||1;
    efBase.sem_ataque_turnos_restantes = efBase.sem_ataque_turnos;
  }
  if (document.getElementById('ar-ef-def-on')?.checked) {
    efBase.mod_defesa        = parseInt(document.getElementById('ar-ef-def-mod').value) || 3;
    efBase.mod_defesa_turnos = parseInt(document.getElementById('ar-ef-def-turnos').value) || 2;
    efBase.mod_defesa_turnos_restantes = efBase.mod_defesa_turnos;
  }
  ...
  for (const charNome of selecionados) {
    ...
    if (!c) continue;
    // Cura imediata
    if (efBase.heal_formula) {
      const grupos = parsearFormulaDano(efBase.heal_formula);
      const r = grupos ? rolarGrupos(grupos) : { total: parseInt(efBase.heal_formula)||0 };
    ...
    if (efBase.rec_atributo && efBase.rec_modo === 'imediato') {
      const grupos = parsearFormulaDano(efBase.rec_formula || '0');
      const r = grupos ? rolarGrupos(grupos) : { total: parseInt(efBase.rec_formula)||0 };
      if (!c.custom_attrs.atributos) c.custom_attrs.atributos = {};
      c.custom_attrs.atributos[efBase.rec_atributo] = (parseFloat(c.custom_attrs.atributos[efBase.rec_atributo])||0) + r.total;
    }
...
async function avancarTurno() {
  ...
  if (AR.estado.cooldowns) {
    for (const id of Object.keys(AR.estado.cooldowns)) {
      AR.estado.cooldowns[id]--;
      if (AR.estado.cooldowns[id] <= 0) delete AR.estado.cooldowns[id];
    }
  }
...
function arRolarDadoSel(){
  ...
  if(el){ el.classList.remove('girar'); void el.offsetWidth; el.classList.add('girar'); el.textContent=r; }
}
...
async function verHistorico(loreId) {
  ...
  const chars = (hd.chars_snapshot||[]).map(c => `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.04)"><div style="width:8px;height:8px;border-radius:50%;background:${c.cor||'#e8604c'}"></div><span style="font-family:'Cinzel',serif;font-size:0.85rem">${c.nome}</span><span style="margin-left:auto;color:${c.hp<=0?'#e74c3c':c.hp<(c.hpMax||100)*0.3?'#f39c12':'#5ee09a'};font-family:'Cinzel',serif;font-size:0.82rem">${c.hp}/${c.hpMax||100}</span></div>`).join('');
  const logs = (hd.log||[]).slice(-20).map(l => `<div style="padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.03);font-size:0.85rem;color:#9a8888"><span style="font-family:'Cinzel',serif;font-size:0.6rem;color:rgba(232,80,60,0.4);margin-right:8px">T${l.turno||0}</span>${l.texto}</div>`).join('');
  document.getElementById('ar-hist-view-conteudo').innerHTML = `
    <div style="margin-bottom:12px"><span style="font-family:'Cinzel',serif;font-size:0.65rem;color:#7a6060">DATA:</span> ${hd.data||'—'} · <span style="font-family:'Cinzel',serif;font-size:0.65rem;color:#7a6060">TURNOS:</span> ${hd.turno_final||0}</div>
...
async function arSalvarEstado() {
  const id = AR.session.rpg_id;
  if (AR.iniciativa) AR.estado.iniciativa_arena = AR.iniciativa;
  else delete AR.estado.iniciativa_arena;
  // Limpar ataques finalizados (evita payload crescer e salvar falhar silenciosamente)
  if (AR.estado.ataques_arena) {
    AR.estado.ataques_arena = AR.estado.ataques_arena.filter(a =>
...
function abrirModalCenario() {
  ...
  document.getElementById('ar-cenario-img').value=AR.estado.cenario_img||'';
  ...
  if (AR.estado.cenario_img && prevEl) { prevEl.src=AR.estado.cenario_img; if(prev)prev.style.display='block'; }
...
function arImportarCenarioJSON() {
  const txt = (document.getElementById('ar-cenario-json')||{}).value||'';
  if (!txt.trim()) return;
  try {
...
function arImportarPropostaCenarioJSON() {
  const txt = (document.getElementById('ar-proposta-cenario-json')||{}).value||'';
  if (!txt.trim()) return;
  try {
    const obj = JSON.parse(txt);
...
document.addEventListener('click', (e) => {
  if (e.target.classList.contains('ar-modal')) fecharModal(e.target.id);
});

// Preview imagem no modal de char
document.addEventListener('input', (e) => {
  if (e.target.id === 'ar-char-img') {
    const thumb = document.getElementById('ar-char-img-thumb');
...
async function arEnviarPropostaCenario() {
  ...
  AR.estado.propostas_cenario.push({
...
async function arAprovarPropostaCenario(id) {
  ...
  if (p.img) AR.estado.cenario_img = p.img;
...
async function arEnviarSolicitacaoEntidade() {
  ...
  if (!nome) { arToast('Informe o nome','erro'); return; }
  if (!AR.estado.solicitacoes_entidade) AR.estado.solicitacoes_entidade = [];
  AR.estado.solicitacoes_entidade.push({
...
function renderSolicitacoesEntidade() {
  ...
  if (!wrap || !list) return;
  const sols = AR.estado.solicitacoes_entidade || [];
  if (AR.myRole !== 'mestre') return;
...
async function arAprovarEntidade(id) {
  const s = (AR.estado.solicitacoes_entidade||[]).find(x=>x.id===id);
  if (!s) return;
  ...
  try {
    ...
    charObj.custom_attrs = customAttrs;
    AR.chars.push(charObj);
    AR.estado.solicitacoes_entidade = (AR.estado.solicitacoes_entidade||[]).filter(x=>x.id!==id);
...
function mesaZoomInit() {
  ...
  wrap.addEventListener('pointerdown', (e) => {
    if (e.target.closest('#ar-mesa-zoom-hud')) return;
    if (MESA.toolMode) return;
    if (e.target.closest('.ar-mesa-token') && e.button === 0) return;
...
function renderMesa() {
  ...
  if (turnoEl) turnoEl.textContent = AR.estado.turno || 0;
...
function mesaOnDrag(e) {
  ...
  if (!MESA._lastBroadcast || _nowM - MESA._lastBroadcast > 50) {
    MESA._lastBroadcast = _nowM;
...
function abrirModalArMapa() {
  const img = AR.estado.cenario_img || '';
  ...
  const t3d = AR.estado.transform3d || {};
